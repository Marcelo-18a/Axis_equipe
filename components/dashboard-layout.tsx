...
export default function DashboardLayout({ children }: DashboardLayoutProps) {
  ...
  return (
    <div className="flex h-screen bg-dark">
      {/* Mobile sidebar */}
      <div
        className={`fixed inset-0 z-40 bg-black bg-opacity-70 transition-opacity lg:hidden ${sidebarOpen ? "opacity-100" : "opacity-0 pointer-events-none"
          }`}
            ...
            {navigation.map((item) => {
              ...
              return (
                <Link
                  key={item.name}
                  href={item.href}
                  className={`flex items-center px-4 py-2 text-sm font-medium rounded-md ${isActive ? "bg-primary text-black" : "text-gray-300 hover:bg-gray-800 hover:text-white"
                    }`}
                >
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <header className="bg-gray-900 shadow-md">
          <div className="flex items-center justify-between h-12 px-4">
            <button
              className="p-0.5 rounded-md text-gray-400 hover:text-white lg:hidden"
              onClick={() => setSidebarOpen(true)}
            >
              <Menu size={20} />
            </button>
            <div className="flex items-center">
              <span className="text-sm text-white font-medium lg:hidden">Dashboard</span>
            </div>
            <div className="flex items-center">
              <button className="p-0.5 rounded-full text-gray-400 hover:text-white">
